...
export interface AgentAudioVisualizerRadialProps {
  ...
  className?: string;
  /**
   * Optional custom volume heights (0-1) to bypass LiveKit track volume.
   */
  volumesOverride?: number[];
}
...
export function AgentAudioVisualizerRadial({
  ...
  const volumeBandsFromTrack = useMultibandTrackVolume(audioTrack, {
    bands: _barCount,
    loPass: 100,
    hiPass: 200,
  });
  ...
  const bands = useMemo(() => {
    if (volumesOverride) return volumesOverride;
    return audioTrack ? volumeBandsFromTrack : new Array(_barCount).fill(0);
  }, [audioTrack, volumeBandsFromTrack, _barCount, volumesOverride]);
